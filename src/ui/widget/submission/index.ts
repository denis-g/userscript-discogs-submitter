...
const COVER_FAILED_MESSAGE = 'Draft created, but cover upload failed!<br /><strong><em>Please review your draft before publishing on Discogs!</em></strong>';

/**
 * Submits the parsed release payload to Discogs and optionally uploads the cover image.
 * Each network step is shown in the loader for at least `MIN_STEP_DURATION_MS` so messages
 * don't flash past the user when the underlying call is fast.
 */
export class SubmissionController {
  private readonly state: WidgetState;
  private readonly submitButton: HTMLElement | null;
   ...
   * @param submitButton - The submit button, hidden while submission is in flight.
   ...
   */
  constructor(state: WidgetState, submitButton: HTMLElement | null, loader: LoaderController, status: StatusController) {
    this.state = state;
  ...
  public async submit(): Promise<void> {
    ...
    try {
      ...
      if (coverUploadFailed) {
        this.status.set(COVER_FAILED_MESSAGE, 'warning');
      }
      else {
