...
(function() {
    'use strict';
    var styles_default$10 = ".discogs-submitter__inject__button{margin-top:24px;margin-right:100%}";
    var styles_default$9 = ".discogs-submitter__inject__button{box-sizing:border-box;margin-bottom:1.5em}";
    var styles_default$8 = ".discogs-submitter__inject__button{margin-top:8px}";
    var styles_default$7 = ".discogs-submitter__header__cover__image{opacity:1}.discogs-submitter__inject__button{margin:1.429rem 0 0}";
    var styles_default$6 = ".discogs-submitter__inject__button{margin:15px 0 0}";
    var styles_default$5 = ".discogs-submitter__inject__button{margin-top:20px}";
    var styles_default$4 = ".discogs-submitter__inject__button{text-transform:none;margin-top:20px}.discogs-submitter__inject__button__icon{margin-top:-4px}";
    ...
    var USERSCRIPT = {
        ...
        VERSION: info?.script?.version || "3.2.6",
    ...
    var qobuz = {
        ...
        injectButton: (button, target) => {
            ...
            if (typeof windowProxy.infiniteScroll === "function") {try {
    ...
    var PROVIDER_STYLES = Object.fromEntries(Object.entries(Object.assign({
        "./7digital/styles.css": "",
        "./amazonmusic/styles.css": styles_default$10,
        "./bandcamp/styles.css": styles_default$9,
        "./beatport/styles.css": styles_default$8,
        "./bleep/styles.css": styles_default$7,
        "./hdtracks/styles.css": styles_default$6,
        "./junodownload/styles.css": styles_default$5,
        "./qobuz/styles.css": styles_default$4
    })).map(([path, content]) => {
    ...
    var InjectButton = class {
        ...
        injectStyles() {
            if (document.getElementById(INJECT_BUTTON_STYLES_ID)) return;
            const style = document.createElement("style");
            style.id = INJECT_BUTTON_STYLES_ID;
            style.textContent = styles_default$3;
            document.head.appendChild(style);
        }
        build() {
            const data = { scriptName: USERSCRIPT.NAME };
            const wrapper = document.createElement("div");
            renderTemplate(template_default$5, data, wrapper);
        ...
        setStore(storeId) {
            if (this.element) {
                ...
                this.element.classList.forEach((className) => {
                    if (className.startsWith("is-")) classesToRemove.push(className);
                });
    ...
    var LoaderController = class {
        ...
        setActive(isActive, label = "Please wait...") {
            ...
            if (isActive) {renderTemplate(template_default$3, {
                thumb: this.getThumb(),
                label
    ...
    var Select = class Select {
        ...
        async buildUI() {
            ...
            const wrapper = document.createElement("div");
            renderTemplate(template_default$2, data, wrapper);
    ...
    async function renderFragment(release, options, container, editedData) {
        ...
        renderTemplate(template_default$1, {
            ...release,
    ...
    var PreviewController = class {
        ...
        bindEvents() {
            ...
            this.contentElement.addEventListener("keydown", (event) => {
                if (event.target.classList.contains("is-edit")) EditableHelper.handleKeydown(event);
            });
    ...
    var StatusController = class {
        ...
        getStore;
        hasParsedData;
        constructor(statusElement, textElement, debugButton, getStore, hasParsedData) {
    ...
    var COVER_FAILED_MESSAGE = "Draft created, but cover upload failed!<br /><strong><em>Please review your draft before publishing on Discogs!</em></strong>";
    var SubmissionController = class {
        state;
        submitButton;
        loader;
        status;
        constructor(state, submitButton, loader, status) {
            this.state = state;
        ...
        async submit() {
            ...
            try {
                ...
                if (coverUploadFailed) this.status.set(COVER_FAILED_MESSAGE, "warning");
    ...
    var Widget = class {
        ...
        async buildPopup() {
            ...
            renderTemplate(template_default, {
        ...
        composeControllers() {
            ...
            this.status = new StatusController(this.elements.status, this.elements.statusText, this.elements.statusButtonDebug, () => this.state.currentDigitalStore, () => Boolean(this.state.originalData));
            ...
            this.submission = new SubmissionController(this.state, this.elements.actionsButtonSubmit, this.loader, this.status);
        }
        async startParsing() {
            ...
            try {
                ...
                if (!data.country) data.country = "Worldwide";
                else data.country = ALLOWED_COUNTRIES.includes(data.country) ? data.country : normalizeCountry(data.country);
        ...
        bindDraggableEvent() {
            if (this.elements.widget && this.elements.headerButtonMove) {new Draggable(this.elements.widget, this.elements.headerButtonMove, (isDragging) => {
    ...
    var SvgSprite = class {
        ...
        buildSymbols() {
            ...
            return Object.entries(ICON_REGISTRY).map(([iconId, svgString]) => {
                ...
                return `<symbol id="${iconId}" viewBox="${svgElement.getAttribute("viewBox") || DEFAULT_VIEWBOX}">${svgElement.innerHTML.trim()}</symbol>`;
            }).filter(Boolean).join("");
        }
    };
    var App = class {
        ...
        bindEvents() {
            if (!this.injectButton.element) return;
            this.injectButton.element.addEventListener("click", () => {
                if (this.injectButton.element?.classList.contains("is-disabled")) return;
