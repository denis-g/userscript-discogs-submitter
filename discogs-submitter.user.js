...
(function() {
    ...
    var InjectButton = class {
        ...
        injectStyles() {
            if (!document.getElementById(`${this.WIDGET_ID}-inject-styles`)) {
                ...
                style.textContent = inject_button_default$1;
        ...
        build() {
            ...
            renderTemplate(inject_button_default, data, wrapper);
    ...
    var UiSelect = class UiSelect {
        ...
        constructor(select) {
            ...
            this.select[UiSelect.DATA_KEY] = this;
        }
        async buildUI() {
            ...
            const wrapper = document.createElement("div");
            renderTemplate(select_default, data, wrapper);
        ...
        async addListItem(value, text, isSelected) {
            ...
            renderTemplate(select_item_default, {
                value,
                text,
                isSelected,
                isMultiple: this.state.isMultiple,
        ...
        static init(select, force = false) {
            if (!select) return;
            const instance = select[UiSelect.DATA_KEY];
            if (instance) {
    ...
    var PreviewRenderer = { releasePreview: async (release, options, domElement, editedData) => {
        ...
        renderTemplate(preview_default$1, {
            ...release,
    ...
    var UiWidget = class {
        ...
        async buildPopup() {
            ...
            renderTemplate(widget_default$1, {
        ...
        reset() {
            if (this.ui.widget) this.ui.widget.classList.remove("is-open");
            this.state.currentPayload = null;
            this.state.lastRawData = null;
            this.state.editedData = null;
            if (this.ui.content) this.ui.content.innerHTML = "";
            this.setStatus("Ready to parse...", "info");
            if (this.ui.actionsButtonSubmit) this.ui.actionsButtonSubmit.setAttribute("hidden", "true");
            this.updateHeaderCover();
        }
        updateHeaderCover() {
            if (!this.ui.cover) return;
            renderTemplate(cover_default, { thumb: this.state.editedData?.thumb }, this.ui.cover, { replace: true });
        }
        setLoader(isActive, label = "Please wait...") {
            if (this.ui.loader) {
                this.ui.loader.classList.toggle("is-loading", isActive);
                if (isActive) {renderTemplate(loader_default$1, {
                    thumb: this.state.editedData?.thumb,
                    label
                }, this.ui.loader, { replace: true });}
            }
        }
        ...
        setStatus(message, status = "info") {
            if (this.ui.statusText) this.ui.statusText.innerHTML = message;
            if (this.ui.status) {
                this.ui.status.classList.remove("is-error", "is-success", "is-info", "is-warning");
                this.ui.status.classList.add(`is-${status}`);
            }
            if (this.ui.statusButtonDebug) {if (status === "error" || status === "success") this.ui.statusButtonDebug.removeAttribute("hidden");
            else this.ui.statusButtonDebug.setAttribute("hidden", "true");}
        }
        async startParsing() {
            ...
            if (this.ui.actionsButtonSubmit) this.ui.actionsButtonSubmit.setAttribute("hidden", "true");
            if (this.ui.content) this.ui.content.innerHTML = "";
            if (this.ui.status) delete this.ui.status.dataset.rawJson;
            try {
                ...
                if (this.ui.status) this.ui.status.dataset.rawJson = `URL: ${unsafeWindow.location.href}\nVersion: ${USERSCRIPT.VERSION}\nError Trace:\n${error.stack || error}`;
            } finally {
        ...
        async renderPayload() {
            ...
            if (this.ui.content) {
                await PreviewRenderer.releasePreview(previewObject, {
                ...
                }, this.ui.content, this.state.editedData);
            ...
            if (this.ui.widget && this.state.editedData.thumb) document.documentElement.style.setProperty("--ds-thumb-url", `url('${this.state.editedData.thumb}')`);
            this.updateHeaderCover();
            if (this.ui.status) this.ui.status.dataset.rawJson = rawJsonString;
            if (this.ui.actionsButtonSubmit) this.ui.actionsButtonSubmit.removeAttribute("hidden");
        }
        async handleDebugCopy() {
            ...
            try {
                ...
                if (this.ui.statusButtonDebug) this.ui.statusButtonDebug.classList.add("is-success");
                setTimeout(() => {
                    if (this.ui.statusButtonDebug) this.ui.statusButtonDebug.classList.remove("is-success");
                    this.setLoader(false);
                }, 2e3);
            } catch {
                if (this.ui.statusButtonDebug) this.ui.statusButtonDebug.classList.add("is-error");
                setTimeout(() => {
                    if (this.ui.statusButtonDebug) this.ui.statusButtonDebug.classList.remove("is-error");
                    this.setLoader(false);
                }, 2e3);
            }
        }
        async handleSubmit() {
            ...
            try {
                ...
                if (coverUploadFailed) this.setStatus(`Draft created, but cover upload failed!<br /><strong><em>Please review your draft before publishing on Discogs!</em></strong>`, "warning");
                else this.restoreReadyState();
            } catch (error) {
        ...
        async runSubmitStep(message, operation) {
            ...
            if (elapsed < MIN_DURATION_MS) await new Promise((resolve) => setTimeout(resolve, MIN_DURATION_MS - elapsed));
            return result;
        }
        ...
        restoreReadyState() {
            if (!this.state.lastRawData) return;
            this.setStatus(this.getParsedStatusMessage(), "success");
        }
        getStoreWarning() {
            if (this.state.currentDigitalStore?.id === "bandcamp") return "<small><strong>Be sure to check the metadata, as formatting can vary significantly between labels and artists.</strong></small>";
        ...
        async handlePreviewChange(event) {
        ...
        async handleRestore(element) {
            const path = this.getResolvedPath(element);
            if (!path || !this.state.lastRawData || !this.state.editedData) return;
        ...
        bindEvents() {
            ...
            this.ui.content?.addEventListener("blur", (event) => {
                if (event.target.classList.contains("is-edit")) this.handlePreviewChange(event);
            }, true);
        ...
        bindDraggableEvent() {
            if (this.ui.widget && this.ui.headerButtonMove) {new Draggable(this.ui.widget, this.ui.headerButtonMove, (isDragging) => {
    ...
    var Widget = class {
      ...
      `;
        ...
        bindEvents() {
            if (this.injectButton.element) {this.injectButton.element.addEventListener("click", () => {
        ...
        handleUrlChange() {
            ...
            if (newUrl === this.currentUrl) return false;
        ...
        checkForUrlChange() {
            if (this.handleUrlChange()) this.scheduleInjection();
        }
        scheduleInjection() {
            [
                100,
                ...
                600,
                1e3
            ].forEach((delay) => setTimeout(() => this.refreshInjection(), delay));
        }
        patchPushState() {
