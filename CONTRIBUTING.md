...
*   **Editable fields** — every field is a native `<textarea>` with `aria-label` so long values wrap instead of overflowing. Values are bound via the `data-value` template directive; fields auto-grow to their content height (`field-sizing: content` with a `scrollHeight` JS fallback in `PreviewController.autosizeTextarea`). Fields are single-line-with-wrap by default (`EditableHelper.handleKeydown` blurs on `Enter`); add the `is-multiline` class for true multi-line fields (release/submission notes) where `Enter` inserts a newline. Native semantics make `role`/`aria-multiline` unnecessary.
